	...
	el.children(".spritescontainer").spritespin({
		...
		onFrame: function (e, data) {
			...
			if (stopAt != 0 && stopAt != undefined) {
				framesCounter++;
				if (framesCounter === (stopAt * smallImages.length)) {
					el.children('.spritescontainer').spritespin('api').stopAnimation();
					framesCounter = 0;
				}
			}
			...
			if (smallImages.length > 0) {
				$('#count').text(`${frame} / ${smallImages.length}`);
			}
			const idx = items.indexOf(data.frame);
			if (imagesDetails.length > 0 && idx != -1) {
				$('#thumb').css({
					"background-image": `url("${imagesDetails[idx]}")`,
		...
		onInit: function (e, data) {
			...
			if (initialRotations > 0) {
				const totalFrames = smallImages.length;
...
$(document).ready(function () {
	$("#reverseButton").click(function () {
		// Obtenez le tableau d'images original
		let imagesLarge = el.data("large");

		// Inversez le tableau d'images original
		imagesLarge.reverse();

		// Mettez à jour l'animation SpriteSpin
		updateSpriteSpin(imagesLarge);
	});
});

function updateSpriteSpin() {
	// obtenir une référence à l'API SpriteSpin
	const api = el.children('.spritescontainer').spritespin('api');

	// détruire l'instance existante
	api.destroy();

	// puis, re-créer l'instance avec la nouvelle séquence d'images
	let isSliderBeingUsed = false;

	// function updateSliderAndFrame(data, value) {
	//   $('.spritespin-slider').val(value);
	//   SpriteSpin.updateFrame(data, value);
	// }
